/* ==========================================================================
   SECTION 1: Terrain / noise generator (TerrainAPI) — was the first inline <script>
   ========================================================================== */
...
/* ==========================================================================
   SECTION 2: Loading-screen killer — was the second inline <script>
   ========================================================================== */
(function(){
  function killLoader(){
...
/* ==========================================================================
   SECTION 3: Main application — astronomy core, 3D scene, UI, animation loop
   ========================================================================== */
// =========================================================================
...
const PLANETS = [
  { key:'mercury', relMass:0.0553, name:'Mercury', type:'Terrestrial planet', color:0x9c9186, emissive:0x1a1712,
    diameterKm:4879, periodDays:88, periodLabel:'88 days', distanceAU:0.39, displayRadius:0.52, moonCount:0,
  ...
  { key:'venus', relMass:0.815, name:'Venus', type:'Terrestrial planet', color:0xc8b090, emissive:0x1a140c,
    // Soft-scaled so Sun (r≈5.4) remains clearly largest; relative sizes preserved among planets
    diameterKm:12104, periodDays:225, periodLabel:'225 days', distanceAU:0.72, displayRadius:0.98, moonCount:0,
  ...
  { key:'earth', relMass:1.0, name:'Earth', type:'Terrestrial planet', color:0x3d7ab5, emissive:0x0a1520,
    diameterKm:12742, periodDays:365.25, periodLabel:'365.25 days (1 year)', distanceAU:1.0, displayRadius:1.05, moonCount:1,
  ...
  { key:'mars', relMass:0.107, name:'Mars', type:'Terrestrial planet', color:0xc46a45, emissive:0x1a0c06,
    diameterKm:6779, periodDays:687, periodLabel:'687 days (1.88 years)', distanceAU:1.52, displayRadius:0.66, moonCount:2,
  ...
  { key:'jupiter', relMass:317.8, name:'Jupiter', type:'Gas giant', color:0xd8b48c, emissive:0x241c12,
    diameterKm:139820, periodDays:4333, periodLabel:'11.86 years', distanceAU:5.2, displayRadius:2.45, moonCount:95,
  ...
  { key:'saturn', relMass:95.2, name:'Saturn', type:'Gas giant', color:0xe4d0a0, emissive:0x241f14,
    diameterKm:116460, periodDays:10759, periodLabel:'29.4 years', distanceAU:9.58, displayRadius:2.15, moonCount:146,
  ...
  { key:'uranus', relMass:14.5, name:'Uranus', type:'Ice giant', color:0x9fd8e0, emissive:0x0d1e20,
    diameterKm:50724, periodDays:30687, periodLabel:'84 years', distanceAU:19.2, displayRadius:1.18, moonCount:28,
  ...
  { key:'neptune', relMass:17.1, name:'Neptune', type:'Ice giant', color:0x4f6fd0, emissive:0x0a1024,
    diameterKm:49244, periodDays:60190, periodLabel:'164.8 years', distanceAU:30.05, displayRadius:1.15, moonCount:16,
...
(function(){
  try {
    ...
    if(el){
      el.textContent = simulatedDate.toLocaleString('en-GB', {
...
const TEXTURE_URLS = {
  ...
  earthClouds:TEX + '2k_earth_clouds.jpg',
...
const TEXTURE_EXTRA = {
  ...
  venus: [TEX2 + '2k_venus_surface.jpg', TEX + '2k_venus_surface.jpg', NASA3D + 'Venus/Venus.jpg'],
  ...
  // Prefer larger NASA / SSS maps first so close-ups are not soft blobs
  mars: [
    NASA3D + 'Mars/Mars.jpg',
    TEX + '2k_mars.jpg',
    TEX2 + '2k_mars.jpg'
  ],
  jupiter: [
    TEX + '2k_jupiter.jpg',
    TEX2 + '2k_jupiter.jpg',
    NASA3D + 'Jupiter/Jupiter.jpg'
  ],
  saturn: [
    TEX + '2k_saturn.jpg',
    TEX2 + '2k_saturn.jpg',
    NASA3D + 'Saturn/Saturn.jpg'
  ],
...
// Optional higher-res upgrades attempted after 2K (GPU / network permitting)
const TEXTURE_8K = {
  mars: [NASA3D + 'Mars/Mars.jpg'],
  jupiter: [TEX + '2k_jupiter.jpg'],
  saturn: [TEX + '2k_saturn.jpg'],
  neptune: [NASA3D + 'Neptune/Neptune.jpg'],
  moon: [TEX + '2k_moon.jpg'],
  earth: [TEX + '2k_earth_daymap.jpg', THREE_TEX_HTTP + 'earth_atmos_2048.jpg']
};
...
const NASA_MOON_TEX = {
  'Moon': [
    TEX + '2k_moon.jpg', TEX2 + '2k_moon.jpg',
  ...
  'Mimas': [NASA3D + 'Saturn%20-%20Mimas/Saturn%20-%20Mimas.jpg', TEX + '2k_moon.jpg'],
  'Enceladus': [NASA3D + 'Saturn%20-%20Enceladus/Saturn%20-%20Enceladus.jpg', TEX + '2k_uranus.jpg'],
  'Tethys': [NASA3D + 'Saturn%20-%20Tethys/Saturn%20-%20Tethys.jpg', TEX + '2k_moon.jpg'],
  'Dione': [NASA3D + 'Saturn%20-%20Dione/Saturn%20-%20Dione.jpg', TEX + '2k_moon.jpg'],
  'Rhea': [NASA3D + 'Saturn%20-%20Rhea/Saturn%20-%20Rhea.jpg', TEX + '2k_moon.jpg'],
  'Titan': [
    NASA3D + 'Saturn%20-%20Titan/Saturn%20-%20Titan.jpg',
    TEX + '2k_venus_atmosphere.jpg',
    TEX + '2k_venus_surface.jpg'
  ],
  'Iapetus': [NASA3D + 'Saturn%20-%20Iapetus/Saturn%20-%20Iapetus.jpg', TEX + '2k_mercury.jpg'],
  'Miranda': [NASA3D + 'Uranus%20-%20Miranda/Uranus%20-%20Miranda.jpg', TEX + '2k_moon.jpg'],
  'Ariel': [NASA3D + 'Uranus%20-%20Ariel/Uranus%20-%20Ariel.jpg', TEX + '2k_moon.jpg'],
  'Umbriel': [NASA3D + 'Uranus%20-%20Umbriel/Uranus%20-%20Umbriel.jpg', TEX + '2k_mercury.jpg'],
  'Titania': [NASA3D + 'Uranus%20-%20Titania/Uranus%20-%20Titania.jpg', TEX + '2k_moon.jpg'],
  'Oberon': [NASA3D + 'Uranus%20-%20Oberon/Uranus%20-%20Oberon.jpg', TEX + '2k_moon.jpg'],
  'Triton': [], // use procedural Voyager-inspired map (NASA still + photogrammetry often non-equirect)
  'Proteus': [TEX + '2k_mercury.jpg', TEX + '2k_moon.jpg'],
  'Phobos': [NASA3D + 'Mars%20-%20Phobos/Mars%20-%20Phobos.jpg', TEX + '2k_mercury.jpg'],
  'Deimos': [NASA3D + 'Mars%20-%20Deimos/Mars%20-%20Deimos.jpg', TEX + '2k_mercury.jpg']
};
...
function loadMoonArchiveTexture(name, onReady){
  ...
  function tryNext(){
    ...
    textureLoader.load(url, function(tex){
      ...
      tex.anisotropy = Math.min(16, maxAnisotropy || 1);
...
// Enhance gas giant albedo maps with subtle band structure if the photo is soft

// Progressive quality upgrade: after primary 2K map is on the mesh, optionally
// try a larger map if GPU MAX_TEXTURE_SIZE is high enough (keeps first paint fast).
function tryUpgradePlanetTexture(key, mat){
  try {
    if(!mat || maxTextureSize < 4096) return;
    const list = (TEXTURE_8K && TEXTURE_8K[key]) ? TEXTURE_8K[key].slice() : [];
    // Prefer NASA larger maps when present in EXTRA
    if(TEXTURE_EXTRA && TEXTURE_EXTRA[key]){
      TEXTURE_EXTRA[key].forEach(function(u){ if(list.indexOf(u) < 0) list.push(u); });
    }
    ...
    function next(){
      ...
      const url = list[i++];
      textureLoader.load(url, function(tex){
        if(!tex || !tex.image || !tex.image.width){ next(); return; }
        // Only upgrade if meaningfully larger than current
        const curW = (mat.map && mat.map.image && mat.map.image.width) || 0;
        if(tex.image.width <= curW * 1.05){ next(); return; }
        if(tex.image.width > maxTextureSize){ next(); return; }
        tex.encoding = THREE.sRGBEncoding;
        tex.generateMipmaps = true;
        tex.minFilter = THREE.LinearMipmapLinearFilter;
        tex.anisotropy = Math.min(8, maxAnisotropy || 1);
        if(key === 'jupiter' || key === 'saturn' || key === 'uranus' || key === 'neptune'){
          tex = enhanceGasGiantMap(tex, key) || tex;
        }
        mat.map = tex;
        mat.color.setHex(0xffffff);
        mat.needsUpdate = true;
        console.info('[tex upgrade]', key, tex.image.width + 'x' + tex.image.height);
      }, undefined, function(){ next(); });
    }
    // Defer so first frame stays smooth
    setTimeout(next, 1200 + Math.random() * 800);
  } catch(e){}
}

function enhanceGasGiantMap(tex, key){
  try {
    ...
    // Soft horizontal band modulation (does not erase photo detail)
    ...
    for(let y = 0; y < h; y++){
      ...
      let mod = 1.0;
      if(key === 'jupiter'){
        mod = 0.92 + 0.1 * Math.sin(lat * Math.PI * 11.0) + 0.04 * Math.sin(lat * Math.PI * 23.0);
      } else if(key === 'saturn'){
        mod = 0.94 + 0.07 * Math.sin(lat * Math.PI * 9.0) + 0.03 * Math.sin(lat * Math.PI * 17.0);
      } else if(key === 'uranus'){
        mod = 0.96 + 0.04 * Math.sin(lat * Math.PI * 5.0);
      } else if(key === 'neptune'){
        mod = 0.95 + 0.05 * Math.sin(lat * Math.PI * 6.0) + 0.02 * Math.sin(lat * Math.PI * 13.0);
      }
      for(let x = 0; x < w; x++){
        const i = (y * w + x) * 4;
        d[i] = Math.min(255, d[i] * mod);
        d[i+1] = Math.min(255, d[i+1] * mod);
        d[i+2] = Math.min(255, d[i+2] * mod);
      }
    }
    // Jupiter Great Red Spot hint (subtle)
    if(key === 'jupiter'){
      const cx = w * 0.62, cy = h * 0.58, rx = w * 0.06, ry = h * 0.045;
      for(let y = 0; y < h; y++){
        for(let x = 0; x < w; x++){
          ...
          if(e < 1){
            const t = 1 - e;
            const i = (y * w + x) * 4;
            d[i] = Math.min(255, d[i] + 28 * t);
            d[i+1] = Math.max(0, d[i+1] - 8 * t);
            d[i+2] = Math.max(0, d[i+2] - 12 * t);
          }
        }
      }
    }
    ctx.putImageData(overlay, 0, 0);
    const out = new THREE.CanvasTexture(cnv);
    out.encoding = THREE.sRGBEncoding;
    out.wrapS = THREE.RepeatWrapping;
    out.wrapT = THREE.ClampToEdgeWrapping;
    out.anisotropy = Math.min(8, maxAnisotropy || 1);
    out.needsUpdate = true;
    return out;
  } catch(e){
    console.warn('[gas enhance]', e);
    return tex;
  }
}
...
function makeFallbackTexture(hexColor){
  // Detailed procedural equirectangular map — never a flat "colored ball"
  const w = 1024, h = 512;
  const cnv = document.createElement('canvas'); cnv.width = w; cnv.height = h;
  const cctx = cnv.getContext('2d');
  const c = new THREE.Color(hexColor);
  const r0 = Math.round(c.r*255), g0 = Math.round(c.g*255), b0 = Math.round(c.b*255);
  // Latitude bands (works for gas giants + subtle variation for rock)
  for(let y=0; y<h; y++){
    const lat = y / h;
    const band = 0.85 + 0.18 * Math.sin(lat * Math.PI * 14 + 0.4)
               + 0.08 * Math.sin(lat * Math.PI * 37)
               + 0.05 * Math.sin(lat * Math.PI * 3);
    const shade = 0.75 + 0.25 * Math.sin(lat * Math.PI); // poles slightly darker
    const rr = Math.min(255, Math.max(0, r0 * band * shade));
    const gg = Math.min(255, Math.max(0, g0 * band * shade));
    const bb = Math.min(255, Math.max(0, b0 * band * shade));
    cctx.fillStyle = 'rgb('+((rr)|0)+','+((gg)|0)+','+((bb)|0)+')';
    cctx.fillRect(0, y, w, 1);
  }
  // Storm / albedo splotches
  for(let i=0;i<120;i++){
    const x=Math.random()*w, y=30+Math.random()*(h-60);
    const rx=20+Math.random()*80, ry=6+Math.random()*22;
    cctx.beginPath(); cctx.ellipse(x,y,rx,ry,0,0,Math.PI*2);
  ...
  for(let i=0;i<12000;i++){
    cctx.fillStyle = Math.random()>0.5 ? 'rgba(0,0,0,0.06)' : 'rgba(255,255,255,0.04)';
    cctx.fillRect(Math.random()*w, Math.random()*h, 1+Math.random()*2, 1);
  }
...
const sunGeo = new THREE.SphereGeometry(5.8, 96, 96);
const sunMat = new THREE.MeshBasicMaterial({ map: sunTexA, color: 0xffffff });
const sunMesh = new THREE.Mesh(sunGeo, sunMat);
sunGroup.add(sunMesh);
const sunGeo2 = new THREE.SphereGeometry(5.85, 96, 96);
...
PLANETS.forEach(p => {
  ...
  // High tessellation so equirectangular maps look smooth, not faceted "balls"
  const sphereSegs = isTerrestrial ? 192 : 128;
  const tex = loadColorTexture(p.key, function(loadedTex){
    if(!loadedTex) return;
    let mapTex = loadedTex;
    if(p.key === 'jupiter' || p.key === 'saturn' || p.key === 'uranus' || p.key === 'neptune'){
      mapTex = enhanceGasGiantMap(loadedTex, p.key) || loadedTex;
    }
    mat.map = mapTex;
    mat.color.setHex(0xffffff); // pure white so photo map drives color (no tint)
    mat.emissiveIntensity = 0.06; // keep night limb readable without washing bands
    mat.needsUpdate = true;
    if(typeof tryUpgradePlanetTexture === 'function') tryUpgradePlanetTexture(p.key, mat);
    if(isTerrestrial && p.key !== 'earth'){
      const dispTex = deriveDisplacementTexture(loadedTex);
      if(dispTex){
        mat.displacementMap = dispTex;
        mat.displacementScale = p.displayRadius * 0.045;
        mat.displacementBias = -p.displayRadius * 0.02;
        mat.needsUpdate = true;
      }
    }
  }, p.color);
  const mat = new THREE.MeshStandardMaterial({
    map: tex,
    color: 0xffffff,
    roughness: (p.key === 'jupiter' || p.key === 'saturn') ? 0.72
             : (p.key === 'uranus' || p.key === 'neptune') ? 0.55
             : (p.key === 'earth' ? 0.62 : 0.82),
    metalness: 0.0,
    emissive: new THREE.Color(p.emissive || 0x111111),
    emissiveIntensity: 0.08
  });
  const geo = new THREE.SphereGeometry(p.displayRadius, sphereSegs, Math.round(sphereSegs * 0.55));
  const mesh = new THREE.Mesh(geo, mat);
  mesh.userData.planet = p;
  // Mercury: ensure dense cratered look (photo map preferred; procedural Messenger-style fallback)
  if(p.key === 'mercury'){
    const mercProc = (typeof makeTypedMoonMap === 'function')
      ? makeTypedMoonMap(0x9c9186, 'mercury', 'Mercury')
      : null;
    if(mercProc){
      // Use procedural until / if photo map loads successfully
      mat.map = mercProc;
      mat.color.setHex(0xffffff);
      mat.emissive = new THREE.Color(0x1a1712);
      mat.emissiveIntensity = 0.1;
      mat.needsUpdate = true;
    }
    loadColorTexture('mercury', function(t){
      if(!t || !t.image) return;
      const iw = t.image.width || 0, ih = t.image.height || 0;
      if(iw < 128 || ih < 64) return;
      t.encoding = THREE.sRGBEncoding;
  ...
  if(p.ring || p.key === 'jupiter' || p.key === 'uranus' || p.key === 'neptune'){
    ...
    p.ring = true;
    // Particle representation of ring material (ice/rock chunks) — Saturn only
    if(p.key === 'saturn'){
      try {
        const N = 4200;
        ...
        const R = p.displayRadius;
        const bands = [
          {inner:1.22, outer:1.52, dens:0.35}, // C
          {inner:1.53, outer:1.95, dens:1.0},  // B
          {inner:2.03, outer:2.27, dens:0.7},  // A
          {inner:2.32, outer:2.36, dens:0.25}  // F
        ];
        let i = 0;
        while(i < N){
          const b = bands[Math.floor(Math.random()*bands.length)];
          if(Math.random() > b.dens) continue;
          const rr = R * (b.inner + Math.random()*(b.outer-b.inner));
          const a = Math.random() * Math.PI * 2;
          const y = (Math.random()-0.5) * R * 0.012;
          positions[i*3]   = Math.cos(a) * rr;
          positions[i*3+1] = y;
          positions[i*3+2] = Math.sin(a) * rr;
          const warm = 0.85 + Math.random()*0.15;
          colors[i*3]   = warm;
          colors[i*3+1] = warm * (0.9 + Math.random()*0.08);
          colors[i*3+2] = warm * (0.75 + Math.random()*0.12);
          i++;
        }
        const pgeo = new THREE.BufferGeometry();
        pgeo.setAttribute('position', new THREE.BufferAttribute(positions, 3));
        pgeo.setAttribute('color', new THREE.BufferAttribute(colors, 3));
        // Soft circular particle sprite (avoids default square points)
        const discCnv = document.createElement('canvas');
        discCnv.width = 64; discCnv.height = 64;
        const dctx = discCnv.getContext('2d');
        const dg = dctx.createRadialGradient(32,32,0,32,32,30);
        dg.addColorStop(0, 'rgba(255,250,240,1)');
        dg.addColorStop(0.45, 'rgba(255,245,230,0.85)');
        dg.addColorStop(1, 'rgba(255,240,220,0)');
        dctx.fillStyle = dg;
        dctx.beginPath(); dctx.arc(32,32,30,0,Math.PI*2); dctx.fill();
        const discTex = new THREE.CanvasTexture(discCnv);
        ...
        ringGroup.add(pts);
        p.ringParticles = pts;
      } catch(e){ console.warn('[rings] particles', e); }
    }
  }
...
const MOONS = [
  ...
  { parent:'saturn', name:'Janus', radius:0.028, dist:7.55, periodDays:0.695, periodLabel:'16.7 hours', diameterKm:179, color:0xc8c0b0,
    discovered:'1966', discoverer:'Audouin Dollfus', fact:'Co-orbital with Epimetheus; the two moons swap orbits every ~4 years.' },
  { parent:'saturn', name:'Epimetheus', radius:0.026, dist:7.52, periodDays:0.694, periodLabel:'16.7 hours', diameterKm:113, color:0xb8b0a0,
    discovered:'1978', discoverer:'Fountain / Larson', fact:'Shares nearly the same orbit as Janus; both help shape the F ring.' },
  { parent:'saturn', name:'Prometheus', radius:0.022, dist:7.4, periodDays:0.613, periodLabel:'14.7 hours', diameterKm:86, color:0xc0b8a8,
    discovered:'1980', discoverer:'Voyager 1', fact:'Inner F-ring shepherd moon; elongated potato shape.' },
  { parent:'saturn', name:'Pandora', radius:0.02, dist:7.48, periodDays:0.629, periodLabel:'15.1 hours', diameterKm:81, color:0xb8b0a0,
    discovered:'1980', discoverer:'Voyager 1', fact:'Outer F-ring shepherd moon.' },
  { parent:'saturn', name:'Helene', radius:0.018, dist:9.08, periodDays:2.737, periodLabel:'2.74 days', diameterKm:35, color:0xd0ccc4,
    discovered:'1980', discoverer:'Laques / Lecacheux', fact:'Trojan moon of Dione (leading Lagrange point).' },
  { parent:'saturn', name:'Telesto', radius:0.016, dist:8.72, periodDays:1.888, periodLabel:'1.89 days', diameterKm:25, color:0xd8d4cc,
    discovered:'1980', discoverer:'Smith et al.', fact:'Trojan moon of Tethys.' },
  { parent:'saturn', name:'Calypso', radius:0.015, dist:8.74, periodDays:1.888, periodLabel:'1.89 days', diameterKm:21, color:0xd0ccc4,
    discovered:'1980', discoverer:'Pascu et al.', fact:'Trailing Trojan of Tethys.' },
  // Uranus
...
(function addMoreMoons(){
  ...
  const satAlready = {Janus:1,Epimetheus:1,Prometheus:1,Pandora:1,Helene:1,Telesto:1,Calypso:1,Atlas:1,Pan:1};
  satOuter.forEach(function(name, i){
    if(satAlready[name]) return;
    MOONS.push({
...
const MOON_API_DATA = {
  ...
  'Triton': { albedo:0.76, surfaceGravity:0.78, meanTempC:-235, textureHint:'triton' },
  ...
  'Tethys': { albedo:0.8, surfaceGravity:0.15, meanTempC:-187, textureHint:'ice' },
  'Dione': { albedo:0.6, surfaceGravity:0.23, meanTempC:-186, textureHint:'ice' },
  'Janus': { albedo:0.7, surfaceGravity:0.01, meanTempC:-195, textureHint:'rocky' },
  'Epimetheus': { albedo:0.7, surfaceGravity:0.01, meanTempC:-195, textureHint:'rocky' }
};
const MOON_API = {
...
function makeMoonTexture(hexColor){
  ...
  for(let i=0;i<28;i++){
    const x=Math.random()*w, y=Math.random()*h, rad=20+Math.random()*90;
    ctx.beginPath(); ctx.ellipse(x,y,rad*1.4,rad*0.7,0,0,Math.PI*2);
    ctx.fillStyle='rgba('+(r*0.5|0)+','+(g*0.5|0)+','+(b*0.5|0)+',0.38)';
    ctx.fill();
  }
...
function makeTypedMoonMap(hexColor, hint, bodyName){
  // Always paint FULL 2:1 equirectangular so spheres never show half-blank hemispheres
  const w = 1024, h = 512;
  const cnv = document.createElement('canvas'); cnv.width = w; cnv.height = h;
  const ctx = cnv.getContext('2d');
  const r = (hexColor >> 16) & 255, g = (hexColor >> 8) & 255, b = hexColor & 255;
  const name = (bodyName || '').toString();

  function noiseDots(n, alphaLo, alphaHi, col){
    for(let i=0;i<n;i++){
      ctx.fillStyle = col || ('rgba(0,0,0,'+(alphaLo+Math.random()*(alphaHi-alphaLo))+')');
      ctx.fillRect(Math.random()*w, Math.random()*h, 1+Math.random()*2, 1);
    }
  }
  function softBlob(x,y,rx,ry,fill){
    ctx.beginPath(); ctx.ellipse(x,y,rx,ry,0,0,Math.PI*2); ctx.fillStyle=fill; ctx.fill();
  }

  if(hint === 'triton'){
    // Triton: pinkish nitrogen ice + darker cantaloupe terrain (Voyager-inspired)
    const grad = ctx.createLinearGradient(0, 0, 0, h);
    grad.addColorStop(0.0, '#c8b0b8');
    grad.addColorStop(0.25, '#e8d0c8');
    grad.addColorStop(0.45, '#d4b8b0');
    grad.addColorStop(0.65, '#a89088');
    grad.addColorStop(0.85, '#8a7870');
    grad.addColorStop(1.0, '#6a5c58');
    ctx.fillStyle = grad; ctx.fillRect(0, 0, w, h);
    for(let i = 0; i < 90; i++){
      const x = Math.random() * w;
      const y = h * 0.45 + Math.random() * h * 0.5;
      const rr = 8 + Math.random() * 28;
      ctx.beginPath(); ctx.arc(x, y, rr, 0, Math.PI * 2);
      ctx.fillStyle = 'rgba(90,75,70,' + (0.18 + Math.random() * 0.2) + ')';
      ctx.fill();
      ctx.beginPath(); ctx.arc(x - rr * 0.15, y - rr * 0.15, rr * 0.35, 0, Math.PI * 2);
      ctx.fillStyle = 'rgba(200,180,170,0.12)'; ctx.fill();
    }
    ctx.fillStyle = 'rgba(240,230,235,0.35)';
    ctx.fillRect(0, 0, w, h * 0.14);
    for(let i = 0; i < 40; i++){
      const x = Math.random() * w, y = Math.random() * h * 0.5;
      ctx.strokeStyle = 'rgba(255,245,250,' + (0.08 + Math.random() * 0.12) + ')';
      ctx.lineWidth = 1 + Math.random() * 2;
      ctx.beginPath(); ctx.moveTo(x, y); ctx.lineTo(x + 10 + Math.random() * 30, y + Math.random() * 8); ctx.stroke();
    }
  } else if(hint === 'haze'){
    // Titan: thick orange organic haze + subtle banding (Cassini-like)
    const grad = ctx.createLinearGradient(0, 0, 0, h);
    grad.addColorStop(0, '#6a4a28');
    grad.addColorStop(0.25, '#c48a42');
    grad.addColorStop(0.5, '#e0a858');
    grad.addColorStop(0.75, '#b87838');
    grad.addColorStop(1, '#5a3c20');
    ctx.fillStyle = grad; ctx.fillRect(0, 0, w, h);
    for(let y = 0; y < h; y++){
      const band = 0.92 + 0.08 * Math.sin(y * 0.07) + 0.04 * Math.sin(y * 0.19);
      ctx.fillStyle = 'rgba(255,200,120,' + (0.04 + 0.03 * Math.sin(y * 0.11)) + ')';
      ctx.globalAlpha = Math.max(0, band - 0.9);
    ...
    for(let i = 0; i < 60; i++){
      const x = Math.random() * w, y = h * 0.35 + Math.random() * h * 0.3;
      ctx.beginPath(); ctx.ellipse(x, y, 30 + Math.random() * 70, 4 + Math.random() * 10, 0, 0, Math.PI * 2);
      ctx.fillStyle = 'rgba(60,35,15,0.2)'; ctx.fill();
    }
    ctx.fillStyle = 'rgba(180,160,200,0.18)';
    ctx.fillRect(0, 0, w, h * 0.12);
    ctx.fillRect(0, h * 0.88, w, h * 0.12);
  } else if(hint === 'volcanic'){
    ctx.fillStyle = 'rgb(' + r + ',' + g + ',' + b + ')'; ctx.fillRect(0, 0, w, h);
    for(let i = 0; i < 80; i++){
      ctx.fillStyle = 'rgba(' + (180 + Math.random() * 60) + ',' + (40 + Math.random() * 40) + ',20,0.35)';
      ctx.beginPath(); ctx.arc(Math.random() * w, Math.random() * h, 3 + Math.random() * 18, 0, 7); ctx.fill();
    }
    for(let i = 0; i < 200; i++){
      ctx.fillStyle = 'rgba(40,20,10,0.25)';
      ctx.fillRect(Math.random() * w, Math.random() * h, 1 + Math.random() * 3, 1);
    }
  } else if(hint === 'ice' || /Ganymede|Europa|Callisto|Enceladus|Tethys|Dione|Rhea|Mimas|Ariel|Titania|Oberon|Miranda/i.test(name)){
    // ===== DETAILED ICY CRUST =====
    // Base cold ice tone
    const grad = ctx.createLinearGradient(0, 0, 0, h);
    grad.addColorStop(0, 'rgb(' + Math.min(255, r + 55) + ',' + Math.min(255, g + 58) + ',' + Math.min(255, b + 62) + ')');
    grad.addColorStop(0.35, 'rgb(' + Math.min(255,r+18) + ',' + Math.min(255,g+20) + ',' + Math.min(255,b+24) + ')');
    grad.addColorStop(0.55, 'rgb(' + r + ',' + g + ',' + b + ')');
    grad.addColorStop(1, 'rgb(' + Math.max(0, r - 28) + ',' + Math.max(0, g - 22) + ',' + Math.max(0, b - 18) + ')');
    ctx.fillStyle = grad; ctx.fillRect(0, 0, w, h);

    // Dark / bright terrain blocks (Ganymede-style sulci vs dark terrain)
    const isGan = /Ganymede/i.test(name);
    const isEur = /Europa/i.test(name);
    const isCal = /Callisto/i.test(name);
    const isEnc = /Enceladus/i.test(name);
    const nBlocks = isGan ? 28 : (isCal ? 18 : 12);
    for(let i=0;i<nBlocks;i++){
      const x = Math.random()*w, y = Math.random()*h;
      const rx = 40 + Math.random()*(isGan?140:90);
      const ry = 25 + Math.random()*70;
      const dark = isCal || (isGan && Math.random()>0.45);
      softBlob(x,y,rx,ry, dark
        ? 'rgba(55,50,48,'+(0.22+Math.random()*0.28)+')'
        : 'rgba(230,235,240,'+(0.10+Math.random()*0.18)+')');
    }

    // Parallel groove / sulci systems (Ganymede bright terrain)
    if(isGan || (!isEur && !isCal && Math.random()>0.3)){
      for(let s=0;s<14;s++){
        const y0 = Math.random()*h;
        const tilt = (Math.random()-0.5)*0.35;
        const len = 80 + Math.random()*220;
        const x0 = Math.random()*w;
        ctx.strokeStyle = 'rgba(210,220,230,'+(0.12+Math.random()*0.2)+')';
        ctx.lineWidth = 1 + Math.random()*2.5;
        for(let k=0;k<5;k++){
          ctx.beginPath();
          ctx.moveTo(x0, y0 + k*3);
          ctx.lineTo(x0 + len, y0 + k*3 + tilt*len);
          ctx.stroke();
        }
      }
    }

    // Europa lineae — long dark cracks across bright ice
    if(isEur){
      for(let i=0;i<55;i++){
        const x0 = Math.random()*w, y0 = Math.random()*h;
        const ang = Math.random()*Math.PI*2;
        const len = 40 + Math.random()*180;
        ctx.strokeStyle = 'rgba(70,55,50,'+(0.25+Math.random()*0.35)+')';
      ...
      for(let i=0;i<10;i++){
        softBlob(Math.random()*w, Math.random()*h, 30+Math.random()*50, 20+Math.random()*35, 'rgba(90,75,65,0.2)');
      }
    }

    // Enceladus tiger stripes (south) + bright pure ice
    if(isEnc){
      ...
      for(let i=0;i<6;i++){
        ...
        ctx.beginPath();
        ctx.moveTo(w*0.25, y);
        ctx.bezierCurveTo(w*0.4, y+6, w*0.6, y-6, w*0.78, y);
        ctx.stroke();
      }
    }

    // Callisto: heavy ancient cratering, little resurfacing
    const craterN = isCal ? 220 : (isGan ? 90 : 70);
    for(let i=0;i<craterN;i++){
      const x = Math.random()*w, y = Math.random()*h;
      const rr = (isCal?1.2:1.5) + Math.random()*(isCal?16:12);
      ctx.beginPath(); ctx.arc(x,y,rr,0,Math.PI*2);
      ctx.fillStyle = 'rgba(30,35,45,'+(0.22+Math.random()*0.3)+')';
      ctx.fill();
      ctx.beginPath(); ctx.arc(x-rr*0.2,y-rr*0.2,rr*0.45,0,Math.PI*2);
      ctx.fillStyle = 'rgba(220,230,240,0.12)'; ctx.fill();
      // faint rim
      ctx.beginPath(); ctx.arc(x,y,rr,0,Math.PI*2);
      ctx.strokeStyle = 'rgba(200,210,220,0.18)'; ctx.lineWidth = 1; ctx.stroke();
    }

    // Fine ice fractures / frost streaks (all ice moons)
    for(let i=0;i<120;i++){
      const x = Math.random()*w, y = Math.random()*h;
    ...
    noiseDots(900, 0.03, 0.1, null);
  } else if(hint === 'twoface'){
    const grad = ctx.createLinearGradient(0, 0, w, 0);
    grad.addColorStop(0, '#1a1512');
    grad.addColorStop(0.45, '#2a221c');
    grad.addColorStop(0.55, '#d8d0c4');
    grad.addColorStop(1, '#e8e4dc');
    ctx.fillStyle = grad; ctx.fillRect(0, 0, w, h);
    ctx.fillStyle = 'rgba(180,170,160,0.35)';
    ctx.fillRect(0, h * 0.48, w, h * 0.04);
    for(let i = 0; i < 50; i++){
      const x = Math.random() * w, y = Math.random() * h, rr = 2 + Math.random() * 12;
      ctx.beginPath(); ctx.arc(x, y, rr, 0, Math.PI * 2);
      ctx.fillStyle = x < w * 0.5 ? 'rgba(0,0,0,0.35)' : 'rgba(255,255,255,0.12)';
      ctx.fill();
    }
  } else if(/Phobos|Deimos/i.test(name) || (hint === 'rocky' && /Phobos|Deimos/i.test(name))){
    // Mars moons: dark gray-brown carbonaceous regolith + heavy cratering
    // Phobos gets Stickney-scale basin; both stay clearly "moon-like" not white paper
    const isPho = /Phobos/i.test(name);
    const baseDark = isPho ? [92, 78, 68] : [105, 92, 80];
    const grad = ctx.createLinearGradient(0,0,0,h);
    grad.addColorStop(0, 'rgb('+(baseDark[0]+18)+','+(baseDark[1]+14)+','+(baseDark[2]+10)+')');
    grad.addColorStop(0.5, 'rgb('+baseDark[0]+','+baseDark[1]+','+baseDark[2]+')');
    grad.addColorStop(1, 'rgb('+(baseDark[0]-18)+','+(baseDark[1]-16)+','+(baseDark[2]-14)+')');
    ctx.fillStyle = grad; ctx.fillRect(0,0,w,h);
    // Dust / albedo mottling
    for(let i=0;i<35;i++){
      softBlob(Math.random()*w, Math.random()*h, 25+Math.random()*70, 15+Math.random()*45,
        'rgba(40,32,28,'+(0.12+Math.random()*0.2)+')');
    }
    // Stickney crater on Phobos (large basin ~half radius)
    if(isPho){
      const cx = w*0.38, cy = h*0.42, R = 55;
      ctx.beginPath(); ctx.arc(cx,cy,R,0,Math.PI*2);
      ctx.fillStyle = 'rgba(35,28,24,0.55)'; ctx.fill();
      ctx.beginPath(); ctx.arc(cx-R*0.2,cy-R*0.2,R*0.45,0,Math.PI*2);
      ctx.fillStyle = 'rgba(120,105,90,0.2)'; ctx.fill();
      // rim
      ctx.beginPath(); ctx.arc(cx,cy,R,0,Math.PI*2);
      ctx.strokeStyle = 'rgba(140,125,110,0.35)'; ctx.lineWidth = 3; ctx.stroke();
      // grooves radiating from Stickney
      for(let k=0;k<10;k++){
        const ang = -0.6 + k*0.18;
        ctx.strokeStyle = 'rgba(50,42,36,0.25)';
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ctx.moveTo(cx+Math.cos(ang)*R*0.9, cy+Math.sin(ang)*R*0.55);
        ctx.lineTo(cx+Math.cos(ang)*R*2.8, cy+Math.sin(ang)*R*1.6);
        ctx.stroke();
      }
    }
    // Dense small craters
    for(let i=0;i<(isPho?160:120);i++){
      const x=Math.random()*w, y=Math.random()*h, rr=1.2+Math.random()*10;
      ctx.beginPath(); ctx.arc(x,y,rr,0,Math.PI*2);
      ctx.fillStyle = 'rgba(25,20,16,'+(0.3+Math.random()*0.35)+')'; ctx.fill();
      ctx.beginPath(); ctx.arc(x-rr*0.25,y-rr*0.25,rr*0.35,0,Math.PI*2);
      ctx.fillStyle = 'rgba(150,135,120,0.12)'; ctx.fill();
    }
    noiseDots(800, 0.04, 0.1, null);
  } else if(hint === 'mercury' || /Mercury/i.test(name)){
    // Messenger-like: warm gray, dense cratering, bright rays
    const grad = ctx.createLinearGradient(0,0,0,h);
    grad.addColorStop(0,'#9a9288'); grad.addColorStop(0.5,'#7a7268'); grad.addColorStop(1,'#5a5248');
    ctx.fillStyle=grad; ctx.fillRect(0,0,w,h);
    for(let i=0;i<40;i++){
      softBlob(Math.random()*w, Math.random()*h, 20+Math.random()*80, 12+Math.random()*50, 'rgba(50,45,40,0.2)');
    }
    for(let i=0;i<280;i++){
      const x=Math.random()*w, y=Math.random()*h, rr=1.5+Math.random()*14;
      ctx.beginPath(); ctx.arc(x,y,rr,0,Math.PI*2);
      ctx.fillStyle='rgba(30,28,24,'+(0.25+Math.random()*0.35)+')'; ctx.fill();
      ctx.beginPath(); ctx.arc(x-rr*0.25,y-rr*0.25,rr*0.4,0,Math.PI*2);
      ctx.fillStyle='rgba(200,195,185,0.12)'; ctx.fill();
    }
    // Ray systems
    for(let i=0;i<6;i++){
      const cx=Math.random()*w, cy=Math.random()*h;
      for(let k=0;k<12;k++){
        const ang=Math.random()*Math.PI*2, len=30+Math.random()*90;
        ctx.strokeStyle='rgba(220,215,200,'+(0.08+Math.random()*0.12)+')';
        ctx.lineWidth=1+Math.random()*1.5;
        ctx.beginPath(); ctx.moveTo(cx,cy); ctx.lineTo(cx+Math.cos(ang)*len, cy+Math.sin(ang)*len*0.6); ctx.stroke();
      }
    }
    noiseDots(1200,0.04,0.12,null);
  } else {
    // Rocky default — full coverage
    const grad = ctx.createLinearGradient(0, 0, 0, h);
    grad.addColorStop(0, 'rgb(' + Math.max(0, r - 30) + ',' + Math.max(0, g - 28) + ',' + Math.max(0, b - 22) + ')');
    grad.addColorStop(0.5, 'rgb(' + r + ',' + g + ',' + b + ')');
    grad.addColorStop(1, 'rgb(' + Math.max(0, r - 30) + ',' + Math.max(0, g - 28) + ',' + Math.max(0, b - 22) + ')');
    ctx.fillStyle = grad; ctx.fillRect(0, 0, w, h);
    for(let i = 0; i < 30; i++){
      const x = Math.random() * w, y = Math.random() * h, rad = 15 + Math.random() * 70;
      ctx.beginPath(); ctx.ellipse(x, y, rad * 1.3, rad * 0.7, 0, 0, Math.PI * 2);
      ctx.fillStyle = 'rgba(' + ((r * 0.5) | 0) + ',' + ((g * 0.5) | 0) + ',' + ((b * 0.5) | 0) + ',0.35)';
      ctx.fill();
    }
    for(let i = 0; i < 200; i++){
      const x = Math.random() * w, y = Math.random() * h, rad = 1.5 + Math.random() * 14;
      const shade = 0.45 + Math.random() * 0.4;
      ctx.beginPath(); ctx.arc(x, y, rad, 0, Math.PI * 2);
      ctx.fillStyle = 'rgba(' + ((r * shade) | 0) + ',' + ((g * shade) | 0) + ',' + ((b * shade) | 0) + ',0.7)';
      ctx.fill();
    }
  }
  const tex = new THREE.CanvasTexture(cnv);
...
(function applyJupiterLinearDistances(){
  ...
  const R = jup ? jup.displayRadius : 2.45;
...
// Final pass: keep moons outside planet/rings AND non-overlapping with siblings
(function separateMoonOrbits(){
  const byParent = {};
  MOONS.forEach(function(m){
  ...
  Object.keys(byParent).forEach(function(pk){
    const parent = PLANETS.find(function(p){ return p.key === pk; });
    if(!parent) return;
    const R = parent.displayRadius;
    const ringF = (typeof RING_OUTER_FACTOR !== 'undefined' && RING_OUTER_FACTOR[pk]) ? RING_OUTER_FACTOR[pk] : 1.15;
    // Sort by orbital distance — NEVER mix aKm (hundreds of thousands) with display dist
    let list = byParent[pk].slice().sort(function(a,b){
      return (a.dist || 0) - (b.dist || 0);
    });
    // Always keep the classic major moons; only cap anonymous outer survey moons
    const KEEP_ALWAYS = {
      Moon:1, Phobos:1, Deimos:1,
      Metis:1, Adrastea:1, Amalthea:1, Thebe:1, Io:1, Europa:1, Ganymede:1, Callisto:1,
      Himalia:1, Elara:1, Pasiphae:1, Sinope:1,
      Mimas:1, Enceladus:1, Tethys:1, Dione:1, Rhea:1, Titan:1, Hyperion:1, Iapetus:1, Phoebe:1,
      Janus:1, Epimetheus:1, Prometheus:1, Pandora:1, Helene:1, Telesto:1, Calypso:1,
      Miranda:1, Ariel:1, Umbriel:1, Titania:1, Oberon:1,
      Proteus:1, Triton:1, Nereid:1
    };
    const maxShow = (pk === 'jupiter') ? 60 : (pk === 'saturn') ? 40 : (pk === 'uranus' || pk === 'neptune') ? 18 : 12;
    let shown = 0;
    list.forEach(function(m){
      if(KEEP_ALWAYS[m.name]){
        m._hideFar = false;
        shown++;
        return;
      }
    ...
    list = list.filter(function(m){ return !m._hideFar; });
    let prevOuter = R * ringF;
    list.forEach(function(m, i){
      const gap = Math.max(0.35, (m.radius || 0.02) * 2.2 + 0.22);
      const minDist = Math.max(R + (m.radius || 0.05) + 0.55, prevOuter + gap);
      // Soft compression for distant irregulars so they don't fly off into confusion
      let target = m.dist || minDist;
      if(target < minDist) target = minDist;
      // Outer moons: compress into a readable shell beyond major moons
      if(i > 6){
        const shell = minDist + (i - 6) * gap * 1.15;
        target = Math.min(target, shell + 0.5);
        target = Math.max(target, minDist);
      }
      // Hard cap so moons stay near parent (avoid "lost in space")
      const maxDist = R * (pk === 'jupiter' ? 14.5 : pk === 'saturn' ? 16 : pk === 'earth' ? 4.5 : 7.5);
      if(target > maxDist) target = maxDist - (list.length - i) * 0.15;
      m.dist = Math.max(minDist, target);
      prevOuter = m.dist + (m.radius || 0.02);
    });
  });
})();

MOONS.forEach(m => {
  if(m._hideFar) return;
  ...
  // ALL moons are globes (spheres). Only Phobos/Deimos/Hyperion-class get mild potato deformation.
  const MAJOR_GLOBE = {
    Moon:1, Io:1, Europa:1, Ganymede:1, Callisto:1,
    Mimas:1, Enceladus:1, Tethys:1, Dione:1, Rhea:1, Titan:1, Iapetus:1,
    Miranda:1, Ariel:1, Umbriel:1, Titania:1, Oberon:1,
    Triton:1, Proteus:1, Nereid:1,
    Amalthea:1, Himalia:1, Elara:1, Metis:1, Adrastea:1, Thebe:1,
    Janus:1, Epimetheus:1, Prometheus:1, Pandora:1, Helene:1, Telesto:1, Calypso:1, Phoebe:1
  };
  const segs = (MAJOR_GLOBE[m.name] || m.radius > 0.05) ? 64 : (m.radius > 0.025 ? 48 : 32);
  // True irregulars only: known potato moons + very small survey objects
  const potatoNames = { Phobos:1, Deimos:1, Hyperion:1, Proteus:1, Amalthea:1, Phoebe:1 };
  const isTinySurvey = !MAJOR_GLOBE[m.name] && m.radius < 0.018 && /^S\/|^Jupiter |^Saturn /i.test(m.name || '');
  const isIrregularRock = !!(potatoNames[m.name] || isTinySurvey);
  let geo;
    if(isIrregularRock && potatoNames[m.name]){
    // Mars moons & similar: mostly spherical with gentle potato deformation
    geo = new THREE.SphereGeometry(m.radius, 48, 40);
    const pattr = geo.attributes.position;
    const isMarsMoon = (m.name === 'Phobos' || m.name === 'Deimos');
    for(let vi=0; vi<pattr.count; vi++){
      let x=pattr.getX(vi), y=pattr.getY(vi), z=pattr.getZ(vi);
      const len=Math.sqrt(x*x+y*y+z*z)||1;
      x/=len; y/=len; z/=len;
      // Phobos/Deimos: subtle triaxial (real Phobos ~13×11×9 km) — still reads as a moon
      const sx = isMarsMoon ? 1.08 : 1.12;
      const sy = isMarsMoon ? 0.92 : 0.88;
      const sz = isMarsMoon ? 0.97 : 0.96;
      let rr = m.radius * (0.94 + 0.06*Math.sin(x*3.8+y*2.4));
      pattr.setXYZ(vi, x*rr*sx, y*rr*sy, z*rr*sz);
    }
    pattr.needsUpdate = true;
    geo.computeVertexNormals();
  } else {
    // Default: perfect globe for Galileans, Titan, etc.
    geo = new THREE.SphereGeometry(m.radius, segs, segs);
  }
  const apiRow = MOON_API_DATA[m.name];
  const hint = (apiRow && apiRow.textureHint)
    || (m.name === 'Titan' ? 'haze'
    : (m.name === 'Io' ? 'volcanic'
    : (m.name === 'Iapetus' ? 'twoface'
    : (m.name === 'Phobos' || m.name === 'Deimos' ? 'rocky'
    : (m.radius >= 0.06 ? 'ice' : 'rocky')))));
  // ALWAYS start with a full-sphere equirectangular map (never half-blank)
  let moonMap;
  if(m.name === 'Moon'){
    moonMap = makeTypedMoonMap(0xc8c4bc, 'rocky', m.name);
  } else if(isIrregularRock){
    moonMap = makeTypedMoonMap(m.color || 0x7a7060, 'rocky', m.name);
  } else {
    moonMap = makeTypedMoonMap(m.color || 0xa0a8b0, hint, m.name);
  }
  ...
  fill.r = Math.min(1, fill.r * 0.45 + 0.18);
  fill.g = Math.min(1, fill.g * 0.45 + 0.18);
  fill.b = Math.min(1, fill.b * 0.45 + 0.2);
  const isMarsMoonMat = (m.name === 'Phobos' || m.name === 'Deimos');
  let mat = new THREE.MeshLambertMaterial({
    map: moonMap,
    color: 0xffffff,
    emissive: isMarsMoonMat ? new THREE.Color(0x2a221c) : fill,
    emissiveIntensity: (m.name === 'Titan' ? 0.22 : (iceLike ? 0.26 : (isMarsMoonMat ? 0.12 : (isIrregularRock ? 0.14 : 0.22))))
  });
  if(isLuna){
    ...
    if(typeof loadMoonArchiveTexture === 'function'){
      loadMoonArchiveTexture('Moon', function(tex){
        ...
        const iw = tex.image.width || 0, ih = tex.image.height || 0;
        if(iw < 64 || ih < 32 || iw < ih * 1.4) return;
        tex.encoding = THREE.sRGBEncoding;
        tex.wrapS = THREE.RepeatWrapping;
        tex.wrapT = THREE.ClampToEdgeWrapping;
        mat.map = tex;
    ...
    loadMoonArchiveTexture(m.name, function(tex){
      if(!tex || !tex.image) return;
      // Reject partial / non-equirectangular maps that leave half the sphere blank
      const iw = tex.image.width || 0, ih = tex.image.height || 0;
      if(iw < 64 || ih < 32 || iw < ih * 1.4){
        console.info('[moon tex] skip non-equirect map for', m.name, iw, ih);
        return;
      }
      tex.encoding = THREE.sRGBEncoding;
      tex.wrapS = THREE.RepeatWrapping;
      ...
      mat.emissiveIntensity = iceLike ? 0.3 : 0.24;
      mat.needsUpdate = true;
    });
  }
  const mesh = new THREE.Mesh(geo, mat);
...
(function buildAsteroidRocks(){ try {
  ...
  // SOLID closed mesh — lumpy asteroid potato (always filled, never hollow)
  ...
  for(let i=0;i<pos.count;i++){
    ...
    const len = Math.sqrt(x*x+y*y+z*z)||1;
    x/=len; y/=len; z/=len;
    // Multi-octave lumpiness + slight elongation — still a closed solid shell
    let r = 0.88
      + 0.10 * Math.sin(x*2.7 + y*1.9)
      + 0.07 * Math.sin(y*4.1 - z*3.3)
      + 0.05 * Math.sin(z*6.2 + x*5.1)
      + 0.04 * Math.sin(x*9.0 + y*7.0 - z*4.0);
    r = Math.max(0.72, Math.min(1.12, r));
    // Flatten a bit so it reads as a rock, not a sphere
    pos.setXYZ(i, x*r*1.05, y*r*0.82, z*r*0.95);
  }
  ...
  for(let i=0;i<ASTEROID_COUNT;i++){
    ...
    asteroidScaleX[i]=size*(0.75+Math.random()*0.55);
    asteroidScaleY[i]=size*(0.55+Math.random()*0.45);
    asteroidScaleZ[i]=size*(0.7+Math.random()*0.5);
...
/** Ultra-realistic asteroid albedo + normals (refined for Pallas) */
function makeRealisticAsteroidMaps(name, hexColor){
  const W = 768, H = 768;
  const cnv = document.createElement('canvas'); cnv.width = W; cnv.height = H;
  const ctx = cnv.getContext('2d');
  const isPallas = /Pallas/i.test(name || '');
  // Pallas is a B-type asteroid: dark blue-gray silicate, slightly greenish-brown
  let br, bg, bb;
  if(isPallas){
    br = 92; bg = 88; bb = 82;
  } else {
    const c = new THREE.Color(hexColor || 0x9a8a78);
    br = (c.r*255)|0; bg = (c.g*255)|0; bb = (c.b*255)|0;
  }
  // Layered rocky base
  const g = ctx.createRadialGradient(W*0.42, H*0.38, 10, W*0.5, H*0.5, W*0.72);
  g.addColorStop(0, 'rgb('+Math.min(255,br+42)+','+Math.min(255,bg+36)+','+Math.min(255,bb+28)+')');
  g.addColorStop(0.45, 'rgb('+br+','+bg+','+bb+')');
  g.addColorStop(1, 'rgb('+Math.max(0,br-48)+','+Math.max(0,bg-44)+','+Math.max(0,bb-38)+')');
  ctx.fillStyle = g; ctx.fillRect(0,0,W,H);
  // Broad albedo provinces
  for(let i=0;i<(isPallas?28:18);i++){
    const x=Math.random()*W, y=Math.random()*H, r=40+Math.random()*(isPallas?120:90);
    const dark = Math.random()>0.42;
    const grd = ctx.createRadialGradient(x,y,0,x,y,r);
    if(dark){
      grd.addColorStop(0,'rgba(22,20,18,0.5)'); grd.addColorStop(1,'rgba(22,20,18,0)');
    } else {
      grd.addColorStop(0,'rgba(165,158,145,0.2)'); grd.addColorStop(1,'rgba(165,158,145,0)');
    }
    ctx.beginPath(); ctx.arc(x,y,r,0,Math.PI*2); ctx.fillStyle=grd; ctx.fill();
  }
  // Dense regolith grain
  for(let i=0;i<(isPallas?45000:28000);i++){
    const a = 0.03 + Math.random()*0.08;
    ctx.fillStyle = Math.random()>0.5 ? ('rgba(0,0,0,'+a+')') : ('rgba(230,220,205,'+(a*0.55)+')');
    const s = 1 + (Math.random()*Math.random())*2.5;
    ctx.fillRect(Math.random()*W, Math.random()*H, s, s);
  }
  const craterList = [];
  function crater(cx,cy,R,depth){
    craterList.push({x:cx,y:cy,r:R,depth:depth});
    const ej = ctx.createRadialGradient(cx,cy,R*0.85,cx,cy,R*1.85);
    ej.addColorStop(0,'rgba(130,122,110,0)');
    ej.addColorStop(0.35,'rgba(145,136,122,'+(0.22*depth)+')');
    ej.addColorStop(1,'rgba(90,82,72,0)');
    ctx.beginPath(); ctx.arc(cx,cy,R*1.85,0,Math.PI*2); ctx.fillStyle=ej; ctx.fill();
    // raised rim highlight
    ctx.beginPath(); ctx.arc(cx,cy,R,0,Math.PI*2);
    ctx.strokeStyle='rgba(195,185,168,'+(0.22+0.28*depth)+')';
    ctx.lineWidth=Math.max(1.2,R*0.055); ctx.stroke();
    // shadowed bowl
    const bowl = ctx.createRadialGradient(cx-R*0.18,cy-R*0.16,0,cx,cy,R);
    bowl.addColorStop(0,'rgba(18,16,14,'+(0.62*depth)+')');
    bowl.addColorStop(0.55,'rgba(28,25,22,'+(0.5*depth)+')');
    bowl.addColorStop(0.88,'rgba(40,36,32,'+(0.2*depth)+')');
    bowl.addColorStop(1,'rgba(20,18,16,0)');
    ctx.beginPath(); ctx.arc(cx,cy,R,0,Math.PI*2); ctx.fillStyle=bowl; ctx.fill();
    if(R>22){
      ctx.beginPath(); ctx.arc(cx,cy,R*0.11,0,Math.PI*2);
      ctx.fillStyle='rgba(150,142,130,0.4)'; ctx.fill();
    }
  }
  // Large + mid + small craters
  for(let i=0;i<(isPallas?12:8);i++){
    crater(Math.random()*W, Math.random()*H, 22+Math.random()*52, 0.72+Math.random()*0.28);
  }
  for(let i=0;i<(isPallas?80:55);i++){
    crater(Math.random()*W, Math.random()*H, 4+Math.random()*16, 0.48+Math.random()*0.4);
  }
  for(let i=0;i<(isPallas?180:120);i++){
    crater(Math.random()*W, Math.random()*H, 1.2+Math.random()*4, 0.35+Math.random()*0.35);
  }
  // Pallas: faint groove / fracture network
  if(isPallas){
    for(let i=0;i<12;i++){
      ctx.strokeStyle = 'rgba(28,24,20,'+(0.12+Math.random()*0.14)+')';
      ctx.lineWidth = 1 + Math.random()*2.5;
      ctx.beginPath();
    ...
    // subtle bluish-gray sheen patches (B-type)
    for(let i=0;i<6;i++){
      const x=Math.random()*W, y=Math.random()*H, r=50+Math.random()*80;
      const grd = ctx.createRadialGradient(x,y,0,x,y,r);
      grd.addColorStop(0,'rgba(90,100,110,0.12)');
      grd.addColorStop(1,'rgba(90,100,110,0)');
      ctx.beginPath(); ctx.arc(x,y,r,0,Math.PI*2); ctx.fillStyle=grd; ctx.fill();
    }
  }
  const albedo = new THREE.CanvasTexture(cnv);
  ...
  let normalMap = null;
  try {
    if(typeof buildCraterHeightField === 'function' && typeof heightToNormalMap === 'function'){
      const height = buildCraterHeightField(W, H, craterList);
      normalMap = heightToNormalMap(height, W, H, isPallas ? 3.8 : 3.2, isPallas ? 0.13 : 0.11);
    }
  } catch(e){}

  return { map: albedo, normalMap: normalMap };
}

function buildSmallBody(a, opts){
  opts = opts || {};
  const isAsteroid = /Asteroid/i.test(a.type || '') || /Pallas|Vesta|Hygiea/i.test(a.name || '');
  const isPallas = /Pallas/i.test(a.name || '');
  let geo;
  if(opts.sphere && !isAsteroid){
    geo = new THREE.SphereGeometry(a.radius, 48, 36);
  } else {
    // Highly detailed irregular solid — potato asteroid (Pallas / Vesta / Hygiea)
    const detail = isPallas ? 4 : 3;
    geo = new THREE.IcosahedronGeometry(a.radius, detail);
    const pos = geo.attributes.position;
    const seed = (a.name || 'x').length * 17.3;
    for(let i=0;i<pos.count;i++){
      let x=pos.getX(i), y=pos.getY(i), z=pos.getZ(i);
      const len=Math.sqrt(x*x+y*y+z*z)||1;
      x/=len; y/=len; z/=len;
      // Multi-octave displacement — still a closed solid shell
      let r = 0.82
        + 0.11 * Math.sin(x*2.9 + y*1.7 + seed)
        + 0.08 * Math.sin(y*4.3 - z*3.1 + seed*0.7)
        + 0.055 * Math.sin(z*6.5 + x*5.2)
        + 0.04 * Math.sin(x*11.0 + y*8.5 - z*4.2);
      r = Math.max(0.68, Math.min(1.14, r));
      // Pallas: more elongated / faceted look
      const sx = isPallas ? 1.14 : 1.08;
      const sy = isPallas ? 0.86 : 0.90;
      const sz = isPallas ? 0.98 : 0.96;
      pos.setXYZ(i, x*r*a.radius*sx, y*r*a.radius*sy, z*r*a.radius*sz);
    }
    pos.needsUpdate = true;
    geo.computeVertexNormals();
  }
  const scaleBoost = opts.dwarf ? 1.35 : (isAsteroid ? 1.15 : 1.2);
  if(opts.sphere && !isAsteroid) geo.scale(scaleBoost, scaleBoost, scaleBoost);
  const mat = new THREE.MeshStandardMaterial({
    color: 0xffffff,
    roughness: isAsteroid ? 0.97 : 0.82,
    metalness: isAsteroid ? 0.02 : 0.04,
    emissive: new THREE.Color(a.color).multiplyScalar(isAsteroid ? 0.12 : 0.25),
    emissiveIntensity: isAsteroid ? 0.12 : 0.3
  });
  // Asteroids: ultra-realistic cratered regolith; dwarfs: typed map
  if(isAsteroid && typeof makeRealisticAsteroidMaps === 'function'){
    const maps = makeRealisticAsteroidMaps(a.name, a.color);
    mat.map = maps.map;
    if(maps.normalMap){
      mat.normalMap = maps.normalMap;
      mat.normalScale = new THREE.Vector2(1.25, 1.25);
    }
  } else if(typeof makeDwarfSurfaceMap === 'function'){
    mat.map = makeDwarfSurfaceMap(a.name, a.color);
  }
  ...
  (function tryDwarfPhoto(bodyName, urls){
    ...
    // Asteroids keep procedural regolith — do not replace with moon photo
    if(/Pallas|Vesta|Hygiea|Asteroid/i.test(a.name+' '+(a.type||''))) return [];
...
MAJOR_ASTEROIDS.forEach(function(a){ buildSmallBody(a, { sphere: a.name === 'Ceres', dwarf: a.name === 'Ceres' }); }); // Pallas/Vesta/Hygiea = irregular asteroid meshes
...
// =========================================================================
// OBSERVER LOCATION — Islamabad default; auto-upgrade via geolocation
// =========================================================================
let OBS_LAT = 33.6844, OBS_LON = 73.0479, OBS_LABEL = 'Islamabad, Pakistan';
(function initObserverLocation(){
  try {
    if(!navigator.geolocation) return;
    navigator.geolocation.getCurrentPosition(function(pos){
      OBS_LAT = pos.coords.latitude;
      OBS_LON = pos.coords.longitude;
      OBS_LABEL = 'Your location (' + OBS_LAT.toFixed(2) + '°, ' + OBS_LON.toFixed(2) + '°)';
      const tags = document.querySelectorAll('#hud-live-block .vh span');
      // refresh open HUD if a planet is focused
      if(focusedPlanet && typeof updatePlanetVisibilityHUD === 'function'){
        updatePlanetVisibilityHUD(focusedPlanet.key);
      }
    }, function(){ /* keep Islamabad */ }, { maximumAge: 6e5, timeout: 8000 });
  } catch(e){}
})();

function findVisibilityWindowAt(planetKey, refDate, lat, lon){
  const localOffsetMs = -new Date().getTimezoneOffset() * 60000; // browser local
  // Prefer PKT when still on default Islamabad
  const usePkt = (Math.abs(lat - 33.6844) < 0.2 && Math.abs(lon - 73.0479) < 0.2);
  const offsetMs = usePkt ? (5*3600*1000) : localOffsetMs;
  const localNow = new Date(refDate.getTime() + offsetMs);
  const Y = localNow.getUTCFullYear(), M = localNow.getUTCMonth(), D = localNow.getUTCDate();
  const steps = [];
  const startMin = 15*60, endMin = 24*60 + 9*60;
  for(let m=startMin; m<=endMin; m+=10){
    const dayOffset = Math.floor(m/1440);
    const hh = Math.floor((m%1440)/60), mm = (m%1440)%60;
    const realUTCms = Date.UTC(Y, M, D+dayOffset, hh, mm, 0) - offsetMs;
    const t = new Date(realUTCms);
    const jd = toJulianDate(t);
    const T = centuriesSinceJ2000(jd);
    const earthPos = heliocentric('earth', T);
    ...
    const pEq = eclipticToEquatorial(pGeo.x, pGeo.y, pGeo.z);
    const planetAlt = altitudeDeg(pEq.raDeg, pEq.decDeg, jd, lat, lon);
    const az = azimuthDeg(pEq.raDeg, pEq.decDeg, jd, lat, lon);
    steps.push({ localLabel: (hh<10?'0':'')+hh+':'+(mm<10?'0':'')+mm, dayOffset, sunAlt, planetAlt, az });
  }
  let bestRun=null, curRun=null;
  for(const s of steps){
    const dark = s.sunAlt < -6 && s.planetAlt > 3;
    if(dark){ curRun = curRun ? { start:curRun.start, end:s, count:curRun.count+1, az:s.az } : { start:s, end:s, count:1, az:s.az }; }
    else { if(curRun){ if(!bestRun||curRun.count>bestRun.count) bestRun=curRun; curRun=null; } }
  }
  if(curRun && (!bestRun||curRun.count>bestRun.count)) bestRun = curRun;
  return bestRun;
...
function updatePlanetVisibilityHUD(planetKey){
  const liveBlock = document.getElementById('hud-live-block');
  const liveContent = document.getElementById('hud-live-content');
  const liveDate = document.getElementById('hud-live-date');
  if(!liveBlock || !liveContent) return;
  liveBlock.style.display = '';
  const vh = liveBlock.querySelector('.vh span');
  if(vh) vh.textContent = 'Sky reference: ' + OBS_LABEL;
  const ref = (typeof simulatedDate !== 'undefined' && simulatedDate) ? simulatedDate : new Date();
  if(liveDate){
    try {
      liveDate.textContent = ref.toLocaleDateString('en-GB', { day:'numeric', month:'short', year:'numeric' });
    } catch(e){ liveDate.textContent = ''; }
  }
  if(planetKey === 'earth'){
    liveContent.innerHTML = '<p style="margin:0;font-size:12px;line-height:1.5;color:var(--parchment)">You are observing from the surface of Earth. Other planets appear against the night sky depending on solar elongation.</p>';
    return;
  }
  const run = findVisibilityWindowAt(planetKey, ref, OBS_LAT, OBS_LON);
  const usePkt = (Math.abs(OBS_LAT - 33.6844) < 0.2 && Math.abs(OBS_LON - 73.0479) < 0.2);
  const tz = usePkt ? 'PKT' : 'local';
  if(!run){
    liveContent.innerHTML = '<p style="margin:0;font-size:12px;line-height:1.55;color:var(--muted)">No long dark-sky window tonight from this location (planet may be near the Sun or below the horizon after twilight). Check again on another date or raise Simulation speed and re-open the panel.</p>';
    return;
  }
  const azMid = run.az != null ? run.az : (run.end && run.end.az);
  const compass = (typeof compassLabel === 'function' && azMid != null) ? compassLabel(azMid) : '—';
  let tag = 'Evening object';
  if(run.start.dayOffset === 1) tag = 'Morning object — visible before dawn';
  else if(run.start.dayOffset !== run.end.dayOffset) tag = 'Visible most of the night';
  else if(parseInt(run.start.localLabel,10) >= 22) tag = 'Late-night object';
  liveContent.innerHTML =
    '<div class="api-row"><span class="k">Best window</span><span class="v">' + run.start.localLabel + ' – ' + run.end.localLabel + ' ' + tz + '</span></div>' +
    '<div class="api-row"><span class="k">Direction</span><span class="v">' + compass + (azMid!=null ? ' (' + Math.round(azMid) + '°)' : '') + '</span></div>' +
    '<div class="api-row"><span class="k">Character</span><span class="v">' + tag + '</span></div>' +
    '<p style="margin:8px 0 0;font-size:11px;color:var(--muted);line-height:1.45">Approximate rise/set from orbital elements for ' + OBS_LABEL + '. Good for planning; not a substitute for a precise almanac.</p>';
}
...
function selectPlanet(key){
  ...
  // Restore planet fact rows
  ['hud-gravity-row','hud-temp-row','hud-density-row','hud-escape-row'].forEach(function(id){
    const el = document.getElementById(id); if(el) el.style.display = '';
  });
  const gEl = document.getElementById('hud-gravity'); if(gEl) gEl.textContent = p.gravity != null ? (p.gravity + ' m/s²') : '—';
  const tEl = document.getElementById('hud-temp'); if(tEl) tEl.textContent = p.meanTempC != null ? (p.meanTempC + ' °C') : '—';
  const dEl = document.getElementById('hud-density'); if(dEl) dEl.textContent = p.density != null ? (p.density + ' g/cm³') : '—';
  const eEl = document.getElementById('hud-escape'); if(eEl) eEl.textContent = p.escapeKmS != null ? (p.escapeKmS + ' km/s') : '—';
  const inclRow = document.getElementById('hud-incl-row'); if(inclRow) inclRow.style.display = '';
  const vis = document.getElementById('hud-visibility');
  if(vis) vis.textContent = p.visibility || '';
  const vb = document.getElementById('hud-vis-block'); if(vb) vb.style.display = '';

  // Live sky window from observer location (Islamabad default, or browser geolocation)
  try {
    if(typeof updatePlanetVisibilityHUD === 'function') updatePlanetVisibilityHUD(p.key);
  } catch(err){ console.warn('[vis HUD]', err); }
...
function focusMoon(m){
  ...
  document.getElementById('hud-subtitle').textContent = (m.discovered ? ('Discovered ' + m.discovered) : '') + (m.discoverer && m.discoverer !== '—' ? ' · ' + m.discoverer : '');
  document.getElementById('hud-diameter').textContent = m.diameterKm ? (m.diameterKm.toLocaleString() + ' km') : '—';
  document.getElementById('hud-period').textContent = m.periodLabel || '—';
  const distLabel = document.getElementById('hud-distance-label');
  if(distLabel) distLabel.textContent = 'Parent body';
  document.getElementById('hud-distance').textContent = parentName || (m.parent || '—');
  const ptypeLabel = document.getElementById('hud-ptype-label');
  if(ptypeLabel) ptypeLabel.textContent = 'Body type';
  document.getElementById('hud-ptype').textContent = 'Natural satellite';
  // Hide planet-only rows; show moon-relevant info
  ['hud-gravity-row','hud-temp-row','hud-density-row','hud-escape-row','hud-incl-row'].forEach(function(id){
    const el = document.getElementById(id); if(el) el.style.display = 'none';
  });
  // Sibling moons of the same parent — click to visit
  const siblings = (typeof MOONS !== 'undefined')
    ? MOONS.filter(function(mm){ return mm.parent === m.parent && mm.mesh; })
    : [];
  const moonsRow = document.getElementById('hud-moons-row');
  if(moonsRow) moonsRow.style.display = siblings.length ? '' : 'none';
  const moonsVal = document.getElementById('hud-moons');
  if(moonsVal) moonsVal.textContent = siblings.length ? (siblings.length + ' in view') : '—';
  const ml = document.getElementById('hud-moon-list');
  if(ml){
    ml.innerHTML = '';
    if(siblings.length){
      ...
      const head = document.createElement('div');
      head.style.cssText = 'width:100%;font-size:9px;letter-spacing:0.1em;text-transform:uppercase;color:var(--muted);margin:0 0 6px;';
      head.textContent = 'Moons of ' + parentName + ' — click to visit';
      ml.appendChild(head);
      siblings.forEach(function(sib){
        const chip = document.createElement('button');
        chip.type = 'button';
        chip.className = 'moon-chip' + (sib === m ? ' active' : '');
        chip.textContent = sib.name;
        chip.title = 'Visit ' + sib.name;
        if(sib === m){
          chip.style.borderColor = 'var(--brass)';
          chip.style.background = 'rgba(201,162,39,0.2)';
  ...
  const api = (typeof MOON_API_DATA !== 'undefined') ? MOON_API_DATA[m.name] : null;
  if(api){
    const gRow = document.getElementById('hud-gravity-row');
    if(gRow && api.surfaceGravity != null){
      gRow.style.display = '';
      const gEl = document.getElementById('hud-gravity');
      if(gEl) gEl.textContent = api.surfaceGravity + ' m/s²';
    }
    const tRow = document.getElementById('hud-temp-row');
    if(tRow && api.meanTempC != null){
      tRow.style.display = '';
      const tEl = document.getElementById('hud-temp');
      if(tEl) tEl.textContent = api.meanTempC + ' °C';
    }
  }
  const vis = document.getElementById('hud-visibility');
  if(vis) vis.textContent = m.fact || ('Natural satellite of ' + parentName + '.');
  const vb = document.getElementById('hud-vis-block'); if(vb) vb.style.display = '';
  const lb = document.getElementById('hud-live-block'); if(lb) lb.style.display = 'none';
  document.getElementById('hud').classList.add('open');
  const visit = document.getElementById('hud-moon-visit');
  ...
  if(typeof startCameraTween==='function'){
    startCameraTween({
      ...
      distance: Math.max(0.4, m.radius*5.5+0.3),
      duration: 1.2
    });
  }
}
...
// Inclination visualization toggle
(function(){
  const btn = document.getElementById('inclbtn');
  const legend = document.getElementById('incl-legend');
  function applyInclState(){
    if(typeof inclVizGroup !== 'undefined' && inclVizGroup){
      inclVizGroup.visible = !!inclinationVisible;
    }
    // Orbit lines stay; inclination markers / ecliptic plane toggle with the button
    if(typeof PLANETS !== 'undefined'){
      PLANETS.forEach(function(p){
        if(p.orbitLine && p.orbitLine.material){
          const I0 = p.inclinationDeg || 0;
          const base = 0.22 + Math.min(1, I0/7.5) * 0.25;
          p.orbitLine.material.opacity = inclinationVisible ? base : Math.max(0.1, base * 0.4);
          p.orbitLine.material.needsUpdate = true;
        }
      });
    }
    if(btn){
      ...
      btn.setAttribute('aria-pressed', inclinationVisible ? 'true' : 'false');
      btn.title = inclinationVisible
        ? 'Hide orbital inclination markers'
        : 'Show orbital inclination markers';
    }
    if(legend){
      legend.style.display = inclinationVisible ? 'block' : 'none';
    }
  }
  if(btn){
    // Avoid double-binding if script re-runs
    if(!btn.dataset.inclBound){
      btn.dataset.inclBound = '1';
  ...
  // Apply once scene is ready
  setTimeout(applyInclState, 0);
  setTimeout(applyInclState, 300);
  setTimeout(applyInclState, 900);
})();
...
if(speedSlider){
  function applySimSpeed(){
    ...
    // v=0 → real-time; v≈18 → ~1 day/s; v=60 → ~1 year/s
    if(vv <= 0.001){
      daysPerSecond = 1/86400;
    } else {
      const t = Math.min(1, Math.max(0, vv / 60));
      const realtime = 1/86400;
      const maxDays = 365.25;
      daysPerSecond = realtime * Math.pow(maxDays / realtime, t);
    }
    if(!isFinite(daysPerSecond) || daysPerSecond <= 0) daysPerSecond = 1/86400;
    const lab = document.getElementById('speedVal');
    if(lab){
      if(vv < 0.05) lab.textContent = '1× real-time';
      else if(daysPerSecond < 1/24) lab.textContent = (daysPerSecond*86400).toFixed(0) + '× real';
      else if(daysPerSecond < 1) lab.textContent = (daysPerSecond*24).toFixed(1) + ' h/s';
...
function animate(now){
  ...
  lastTime = now;
  frameCount++;
  // Always show simulated clock (even when paused) — every frame for snappy feedback
  try {
    const el = document.getElementById('simdate-value');
    if(el && simulatedDate && ((frameCount % 4) === 0)){
      el.textContent = simulatedDate.toLocaleString('en-GB', {
        year:'numeric', month:'short', day:'numeric', hour:'2-digit', minute:'2-digit', hour12:false, timeZone:'UTC'
      }) + ' UTC';
    }
  ...
  if(playing){
    ...
    // Keep top-bar simulated date in sync
    (function(){
      try {
        const el = document.getElementById('simdate-value');
        if(!el) return;
        const d = simulatedDate;
        const opts = { year:'numeric', month:'short', day:'numeric', hour:'2-digit', minute:'2-digit', hour12:false, timeZone:'UTC' };
        el.textContent = d.toLocaleString('en-GB', opts) + ' UTC';
      } catch(e){}
    })();
